...
class SettingsViewModel : ViewModel() {
    ...
    fun updateProfile(token: String?, inputUpdateProfile: InputUpdateProfile) {
        viewModelScope.launch {
            val output = repository.updateProfile(token, inputUpdateProfile)
            _outputUpdateProfile.emit(output)
        }
    }
}